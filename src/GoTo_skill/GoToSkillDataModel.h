#ifndef GOTODATAMODEL_H
#define GOTODATAMODEL_H
...
struct Connector
{
            ...
            m_carrier(std::move(carrier))
    {
        if (!yarp::os::Network::connect(m_from, m_to, carrier)) {
            qFatal("Error! Could not connect to server /fakeGoToServer");
        }
    }
    ...
    ~Connector()
    {
        if (!yarp::os::Network::disconnect(m_from, m_to)) {
            qFatal("Error! Could not disconnect from server /fakeGoToServer");
        }
    }
