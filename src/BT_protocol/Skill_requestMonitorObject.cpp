...
//         yCInfo(SKILLREQUESTMONITOR) << "Sending reply:" << data.toString();
    ...
    if (const auto* reply = thing.cast_as<Skill_request_request_ack_helper>()) {
        yCDebug(SKILLREQUESTMONITOR) << "Received reply to 'request_ack':" << SkillAckVocab().toString(reply->s_return_helper);
        breply.addString("request_ack");
        breply.addInt32(static_cast<int32_t>(reply->s_return_helper));
