...
function(_YARP_IDL_TO_DIR_GENERATE _family _file _name _index_file_name _output_dir _include_prefix _cpp_placement_dir _h_placement_dir _cpp_placement_ns _verbose)
  ...
  if("${_family}" STREQUAL "scxml")
    _scxml_args(_YITD "${_temp_dir}" ${_verbose} _args)
  endif()
...
function(ADD_SKILLS var)
  ...
  foreach(file ${_files})
    ...
    # will be generated
    set(native 0)
    if("${ext}" STREQUAL ".scxml")
      set(family scxml)
      _scxml_to_file_list("${file}" "${path}" "${basename}" ${ext} gen_srcs gen_hdrs 1 1)
    else()
      message(FATAL_ERROR "Unknown extension ${ext}. Supported extension is .scxml")
    endif()
    ...
    file(MAKE_DIRECTORY "${out_dir}") # crea se non esiste già

    # Prepare main command
    if("${family}" STREQUAL "scxml")
      set(cmd ${scxml_COMMAND} --gen yarp:include_prefix,no_namespace_prefix --I "${CMAKE_CURRENT_SOURCE_DIR}" --out "${out_dir}" "${file}")
    endif()
